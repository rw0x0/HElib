...
int test_basic_math_operations(const NTL::ZZ& p1, const NTL:: ZZ& p2,const helib::PubKey& public_key, const helib::SecKey& secret_key, const NTL::ZZ& p) {
  int error = 0;
...

  {
...

   {
    ...
    std::cout << std::endl;
    std::cout << "Multiplying the two ciphertexts..." << std::endl;
    auto ctxt3 = ctxt1;
    ctxt3 *= ctxt2;
    std::cout << "Noise budget in ctxt3: " << ctxt3.bitCapacity() << std::endl;
...
int main(int argc, char* argv[])
{
  ...
  NTL::ZZ p2 = NTL::ZZ(10);
  std::cout << std::endl;
  std::cout << "Test with p1 = " << p1 << " and p2 = " << p2 << std::endl;
  auto res = test_basic_math_operations(p1, p2, public_key, secret_key, p);
  if (res == 0) {
  ...
  std::cout << std::endl;
  std::cout << "Test with p1 = " << p1 << " and p2 = " << p2 << std::endl;
  res = test_basic_math_operations(p1, p2, public_key, secret_key, p);
  if (res == 0) {
    std::cout << "All tests passed!" << std::endl;
  } else {
    std::cout << "A test failed..." << std::endl;
